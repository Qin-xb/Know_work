...
project_path = '/data/data/com.termux/files/usr/var/lib/proot-distro/installed-rootfs/ubuntu/root/edge_mm/pictureFinding'

os.makedirs(os.path.join(project_path,'tmp'), exist_ok=True)
os.makedirs(os.path.join(project_path,'embedding'), exist_ok=True)
...
def box(x1, y1, x2, y2, path):
    ...
    url = "http://192.168.40.129:8008/box"
...
def image_index(query):
    data = {"query": query}
    url = "http://192.168.40.129:15003/imageSearch"
...
def pic2embed(image_path):
    ...
    url = "http://192.168.40.129:15002/picture2embedding"
...
def upload_image():
    clip_embedding = np.load(f'{project_path}/embedding/clip.npy')
    e5_embedding = np.load(f'{project_path}/embedding/e5.npy')
    image_name = gen_name()
    st.markdown(
        """
    ...
    if bg_image:
        ...
        with s1:
            if st.button('Click to save original image'):
                _image.save(f"{project_path}/pictures/{image_name}.jpg")
                embed = pic2embed(f"{project_path}/pictures/{image_name}.jpg")
                ...
                np.save(f'{project_path}/embedding/clip.npy', clip_embedding)
                np.save(f'{project_path}/embedding/e5.npy', e5_embedding)

                with open(f'{project_path}/embedding/description.txt', 'a', encoding='utf-8') as file:
                    file.write(discription)
                with open(f'{project_path}/embedding/images.txt', 'a', encoding='utf-8',newline='\n') as file:
                    file.write(f'{project_path}/pictures/{image_name}.jpg'+'\n')
        ...
        if canvas_result.json_data is not None:
            ...
            if num >=1:
                ...
                _image.save(f'{project_path}/tmp/tmp.jpg')
                image_path = f"{project_path}/tmp/tmp_crop.jpg"
                img = box(x1, y1, x2, y2, f"{project_path}/tmp/tmp.jpg")
                if canvas_result.image_data is not None:
                    ...
                    with s2:
                        ...
                        if st.button('Click to save cropped image'):
                            # 保存到库里的目录
                            save_path = f"{project_path}/pictures/{image_name}_crop.jpg"
                            ...
                            with open(f'{project_path}/embedding/description.txt', 'a', encoding='utf-8') as file:
                                file.write(discription)
                            with open(f'{project_path}/embedding/images.txt', 'a', encoding='utf-8',newline='\n') as file:
                                file.write(f'{project_path}/pictures/{image_name}_crop.jpg'+'\n')
